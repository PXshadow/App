...
namespace extension_ios_firebase {
    ...
    static void initFirebase() {
        NSLog(@"Firebase app delegate init started");
        [[FirebaseAppDelegate sharedInstance] initFirebase];
    }
	
	static void registerTopic(value topic) {
		[[FirebaseAppDelegate sharedInstance] registerTopic:[NSString stringWithUTF8String:val_string(topic)]];
	}
}